...
**Feature Branch**: `016-ingredient-phrase-segment`  
**Domain Context**: `ingredient-normalization-validation`  
**Target Domain Folder**: `specs/domains/ingredient-normalization-validation`  
**Created**: 2026-05-06  
**Status**: Draft  
**Input**: User description: "Après une photo d'étiquette, le texte reconnu est disponible ; isoler la liste d'ingrédients par une règle unique : première occurrence d'une phrase commençant par Ingrédient(s) (casse et singulier/pluriel tolérés), fin de liste = fin de phrase si ponctuation finale présente, sinon fin de ligne, sinon fin du texte utile sur la même plage ; plusieurs occurrences → seule la première compte ; toute règle antérieure basée sur une autre ancre ou sur le premier saut de ligne systématique est abrogée."
...
**Independent Test**: À partir d'échantillons de texte représentatifs (ancre française, **ancres anglaises « Ingredient » et « Ingredients »**, plusieurs mentions, avec ou sans ponctuation de fin de phrase), vérifier que la zone proposée pour la liste respecte exactement la règle d'ancrage et de fin décrite dans les exigences.

**Acceptance Scenarios**:

1. **Given** un texte contenant une phrase qui commence par une forme reconnue du mot-ancre (« Ingrédient », « Ingrédients », « Ingredient » ou « Ingredients ») avec des variations raisonnables de casse et de mise en forme, **When** le système prépare la proposition de liste, **Then** la proposition commence au début de cette phrase et se termine à la fin de phrase si une ponctuation de fin de phrase est présente dans cette phrase.
2. **Given** une phrase-ancre sans ponctuation de fin de phrase reconnue dans le texte, **When** le système délimite la proposition, **Then** la proposition se termine à la fin de la ligne où s'étend cette phrase (premier saut de ligne après le début de la phrase-ancre).
3. **Given** une phrase-ancre sans ponctuation de fin de phrase ni saut de ligne applicable, **When** le système délimite la proposition, **Then** la proposition se termine à la fin du fragment de texte continu pertinent sur la même plage (fin du texte fourni pour ce cas).
4. **Given** plusieurs occurrences d'une mention « ingrédients » dans le texte, **When** le système choisit l'ancre, **Then** seule la première occurrence rencontrée en lisant le texte de l'étiquette dans l'ordre naturel sert à la proposition.
5. **Given** un texte dont la phrase-ancre commence par « Ingredients » (forme anglaise, avec variations raisonnables de casse), **When** le système prépare la proposition, **Then** les mêmes règles de délimitation (fin de phrase, fin de ligne, fin de fragment) s'appliquent comme pour une ancre française.
...
- Ponctuation atypique ou collée aux mots : la fin de phrase n'est reconnue que pour les signes de fin de phrase standard (point, point d'exclamation, point d'interrogation) lorsqu'ils jouent ce rôle en fin de segment proposé.
...
- **FR-003**: Le système MUST délimiter la fin de la proposition à la **fin de la phrase** contenant cette ancre, lorsqu'une ponctuation de fin de phrase standard (point, point d'exclamation, point d'interrogation) apparaît dans cette phrase après l'ancre.
- **FR-004**: Lorsqu'aucune ponctuation de fin de phrase n'est présente dans cette phrase, le système MUST délimiter la fin de la proposition au **saut de ligne** qui termine la ligne sur laquelle s'étend la phrase-ancre (fin de ligne).
...
- **SC-001**: Pour un jeu de cas de test documenté couvrant ancre unique, ancres multiples, **au moins un cas avec ancre « Ingredient » et un cas avec ancre « Ingredients »**, et combinaisons ponctuation / saut de ligne / texte monoligne, **100 %** des propositions automatiques respectent les règles FR-002 à FR-006 (vérification par contrôle manuel ou jeu de référence agréé).
