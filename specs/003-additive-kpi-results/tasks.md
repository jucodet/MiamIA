...
**Input**: Design documents from `/home/ju/dev/projects/MiamIA/specs/003-additive-kpi-results/`
...
- **Mobile (ce dépôt)** : `app/src/main/java/com/miamia/`, `app/src/test/java/com/miamia/`, `app/src/androidTest/java/com/miamia/`

## Phase 1: Setup (Shared Infrastructure)

**Purpose**: Créer le module domaine `additives` et les types partagés par toutes les user stories.

- [x] T001 Créer `AdditiveRiskLevel.kt` et `AdditiveLineConfidence.kt` dans `app/src/main/java/com/miamia/additives/` selon `specs/003-additive-kpi-results/data-model.md`
- [x] T002 [P] Créer `AdditiveRiskItem.kt` dans `app/src/main/java/com/miamia/additives/AdditiveRiskItem.kt` selon `specs/003-additive-kpi-results/data-model.md`
- [x] T003 [P] Créer `RiskSummaryKpi.kt` et `AnalysisDisplayResult.kt` dans `app/src/main/java/com/miamia/additives/` selon `specs/003-additive-kpi-results/data-model.md`
...
- [x] T004 Implémenter `AdditiveKpiParser.kt` dans `app/src/main/java/com/miamia/additives/AdditiveKpiParser.kt` (motif `NIVEAU|nom|justification`, repli prudent, normalisation dédoublonnage, tri HIGH→MEDIUM→LOW→UNKNOWN, flags `AdditiveLineConfidence`) selon `specs/003-additive-kpi-results/research.md` et FR-001/FR-003/FR-005
- [x] T005 Implémenter `BuildAdditiveKpiDisplay.kt` dans `app/src/main/java/com/miamia/additives/BuildAdditiveKpiDisplay.kt` avec signature et invariants `RiskSummaryKpi` (SC-003) décrits dans `specs/003-additive-kpi-results/contracts/additive-kpi-ui-contract.md`
- [x] T006 Mettre à jour `app/src/main/java/com/miamia/composition/GemmaBilanParser.kt` pour exclure du champ `compositionAnalysis` tout bloc délimité après `###ANALYSE` (ex. `###ADDITIFS_RISQUE`) si le prompt introduit ce marqueur, et étendre ou ajuster `app/src/test/java/com/miamia/composition/GemmaBilanParserTest.kt` pour couvrir le cas avec et sans marqueur additifs
- [x] T007 [P] Étendre le prompt composition pour le bloc structuré additifs dans `app/src/main/java/com/miamia/composition/LiteRtGemmaEngine.kt` selon `specs/003-additive-kpi-results/research.md`
- [x] T008 [P] Aligner le prompt composition côté client local dans `app/src/main/java/com/miamia/gemma4local/AndroidGemma4LocalGateway.kt` avec les mêmes exigences de format que T007
...
- [x] T009 [P] [US1] Test ordre HIGH→MEDIUM→LOW→UNKNOWN et pastille/domain pour chaque ligne dans `app/src/test/java/com/miamia/additives/US1AdditiveRankingAndBadgesContractTest.kt` (fixtures alignées scénarios US1.1–US1.2)
- [x] T010 [P] [US1] Test état vide `isEmptyState` et message cohérent dans `app/src/test/java/com/miamia/additives/US1AdditiveEmptyStateContractTest.kt` (scénario US1.3)

### Implementation for User Story 1

- [x] T011 [US1] Exposer le résultat `AnalysisDisplayResult` pour `ScanState.BilanReady` via calcul sur `Dispatchers.Default` dans `app/src/main/java/com/miamia/camera/CameraViewModel.kt` en appelant `BuildAdditiveKpiDisplay` avec `bilan` et `rawTranscript` de `app/src/main/java/com/miamia/camera/ScanState.kt`
- [x] T012 [P] [US1] Créer composants Compose liste + pastille + libellé niveau + `contentDescription` dans `app/src/main/java/com/miamia/additives/ui/AdditiveKpiPanel.kt` et `app/src/main/java/com/miamia/additives/ui/AdditiveRiskLine.kt` (contrat liste § « Liste » dans `specs/003-additive-kpi-results/contracts/additive-kpi-ui-contract.md`)
- [x] T013 [US1] Intégrer le panneau KPI sous le bilan existant dans la branche `is ScanState.BilanReady` de `app/src/main/java/com/miamia/camera/CameraScreen.kt` en consommant l’état exposé par `CameraViewModel`
- [x] T014 [US1] Afficher badges « à confirmer » / « incohérence » (et fusion doublons si exposé) sur les lignes lorsque `confidence` ≠ `OK` dans `app/src/main/java/com/miamia/additives/ui/AdditiveRiskLine.kt` (FR-005, contrat erreurs)
...
- [x] T015 [P] [US2] Test troncature + règle `UNKNOWN` pour texte justification dans `app/src/test/java/com/miamia/additives/US2JustificationPresentationContractTest.kt` (scénarios US2.1–US2.2)

### Implementation for User Story 2

- [x] T016 [US2] Appliquer troncature (seuil ~120 caractères, ellipse) côté présentation dans `app/src/main/java/com/miamia/additives/ui/AdditiveRiskLine.kt` selon `specs/003-additive-kpi-results/research.md`
- [x] T017 [US2] Ajouter interaction d’expansion / repli inline (même ligne, pas nouvelle destination) pour le détail justification dans `app/src/main/java/com/miamia/additives/ui/AdditiveRiskLine.kt` (FR-006)
...
- [x] T018 [P] [US3] Test invariant SC-003 et correspondance compteurs / liste dans `app/src/test/java/com/miamia/additives/US3KpiSummaryMatchesListContractTest.kt` (scénarios US3.1–US3.2)

### Implementation for User Story 3

- [x] T019 [US3] Implémenter le bloc synthèse (`totalCount`, `highCount`, `mediumCount`, `lowCount`, `unknownCount`) avec `Modifier.testTag` sur les compteurs dans `app/src/main/java/com/miamia/additives/ui/AdditiveKpiSummaryBar.kt` (nouveau fichier) et l’intégrer depuis `app/src/main/java/com/miamia/additives/ui/AdditiveKpiPanel.kt`
- [x] T020 [US3] Afficher `globalLevel` lorsque non nul dans `app/src/main/java/com/miamia/additives/ui/AdditiveKpiSummaryBar.kt` selon `specs/003-additive-kpi-results/data-model.md`
...
- [x] T021 [P] Afficher une bannière non bloquante si `parseErrors` non vide dans `app/src/main/java/com/miamia/additives/ui/AdditiveKpiPanel.kt` (contrat `data-model.md`)
- [x] T022 [P] Test Compose (liste ordonnée + tags KPI) dans `app/src/androidTest/java/com/miamia/additives/AdditiveKpiPanelComposeTest.kt` aligné sur `specs/003-additive-kpi-results/quickstart.md`
- [x] T023 [P] (Optionnel) Micro-benchmark ou test de performance parsing sur `Dispatchers.Default` dans `app/src/test/java/com/miamia/additives/AdditiveKpiParserPerformanceTest.kt` pour objectif p95 documenté dans `specs/003-additive-kpi-results/plan.md`
